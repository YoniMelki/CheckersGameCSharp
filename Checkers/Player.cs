...
namespace Checkers
{
    public class Player
    {
        ...
        public bool CanEatMore(Board i_Board, Point i_PointWherePawnMoved)
        {
            int coordinateColIndex = i_PointWherePawnMoved.X;
            int coordinateRowIndex = i_PointWherePawnMoved.Y;
        ...
        private void addMoveToMovesList(ref List<Move> o_MovesList, int i_ColNum, int i_RowNum, eMove i_MoveInCol, eMove i_MoveInRow)
        {
            Point source = new Point(i_ColNum, i_RowNum);
            Point destination = new Point(i_ColNum + (int)i_MoveInCol, i_RowNum + (int)i_MoveInRow);
            Move validMove = new Move(source, destination);
            o_MovesList.Add(validMove);
        }
